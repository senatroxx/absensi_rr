...
$id = $_GET['id'];
$data = mysqli_query($koneksi, "SELECT * FROM tbl_gaji WHERE id='$id'");
        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12">
                            <h1 class="page-header">Edit Pembayaran Gaji</h1>
                            <a class="btn btn-primary" style="margin-bottom:20px" href="gaji.php">Kembali</a>
                        </div>
                    </div>
                    <div class="row">
                    <form action="updateGaji.php" method="post">
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="nip" id="selectauto" onchange="auto()">
                                    ...
                                    ?>
                                    <option value="<?= $kar['nip'] ?>" <?php if ($kar['nip'] == $d['nip']) { echo "selected"; } ?>><?= $kar['nip']." - ".$kar['nama'] ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="tanggal">Tanggal</label>
                                <input class="form-control" value="<?php echo $d['tanggal'] ?>" type="date" name="tanggal" id="">
                            </div>
                            <div class="form-group">
                                <label for="gajiPokok">Gaji Pokok</label>
                                <input class="form-control" type="text" readonly value="<?= $d['gaji_pokok'] ?>" name="gajiPokok" id="gajiPokok">
                            </div>
                            <div class="form-group">
                                <label for="gajiHarian">Gaji Harian</label>
                                <input class="form-control" type="number" readonly value="<?= $d['gaji_harian'] ?>" name="gajiHarian" id="gajiHarian">
                            </div>  
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label for="bonus">Bonus</label>
                                <input type="number" class="form-control" name="bonus" id="bonus" value="<?= $d['bonus'] ?>" name="bonus" onkeyup="auto2()" value="0">
                            </div>
                            <div class="form-group">
                                <label for="totalGaji">Total Gaji</label>
                                <input class="form-control" type="number" name="totalGaji" id="totalGaji" value="<?= $d['total'] ?>" name="totalGaji" readonly>
                            </div>
